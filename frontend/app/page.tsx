...
type RunState = { task_id: string | null; session_id: string; scenario_id: number; status: "QUEUED" | "RUNNING" | "SUCCEEDED" | "FAILED"; polling_state?: PollingState; should_poll?: boolean; terminal?: boolean; logs: RunLog[]; simulation_artifacts?: SimulationArtifact[]; agent_breakdown?: AgentBreakdown[]; collective_reasoning?: CollectiveReasoning | null; disagreements?: Disagreement[]; progress_stage?: string | null; result?: { task?: Record<string, unknown>; result?: Record<string, unknown>; why?: Record<string, unknown>; car?: { hard_stop?: { triggered?: boolean; reason?: string; simulation_bypassed?: boolean; llm_compromise_bypassed?: boolean }; rejected_alternatives?: unknown[]; hard_constraints?: unknown[] } } | null; error?: string | null; created_at?: string | null; started_at?: string | null; completed_at?: string | null };
...
function InfeasiblePanel({ metric, hardStop, rejectedAlternatives, hardConstraints }: { metric: Metric | null; hardStop?: { triggered?: boolean; reason?: string; simulation_bypassed?: boolean; llm_compromise_bypassed?: boolean }; rejectedAlternatives?: unknown[]; hardConstraints?: unknown[] }) {
  const { t, formatNumber } = useI18n();
...
export default function Home() {
  ...
  async function startRun() {
    ...
    setRunHydrationError(null);
    setRun({ task_id: payload.task_id, session_id: payload.session_id, scenario_id: payload.scenario_id, status: payload.status, polling_state: payload.polling_state, should_poll: payload.should_poll, terminal: payload.terminal, logs: payload.logs, simulation_artifacts: [] });
